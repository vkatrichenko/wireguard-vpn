...
- **As the operator, I want** the dashboard to watch for the failures that actually matter, **so that I'm** told before they become incidents.
  - **Acceptance Criteria:**
    - [ ] **Service down:** `wg-quick@wg0` is not active → alert.
    - [ ] **High disk:** any monitored filesystem ≥ a disk threshold (default 90%) → alert.
    - [ ] **High CPU sustained:** CPU ≥ a CPU threshold (default 90%) continuously for a sustain window (default 5 minutes) → alert (a brief spike must not fire).
    - [ ] **Peer down:** a client that was online goes stale beyond the peer-stale threshold (default 10 minutes) → alert. `[NEEDS CLARIFICATION: should peer-down alerts apply to all clients or an opt-in subset, to avoid noise from intermittent peers? Proposed: all clients, but only after having been seen online at least once this run.]`
    - [ ] Thresholds and windows are configurable with the documented defaults above.

### 2.2 Delivery

- **As the operator, I want** alerts in my chat, **so that I** see them without opening the dashboard.
  - **Acceptance Criteria:**
    - [ ] Each alert is sent as an HTTPS POST to a configured **Slack-compatible incoming-webhook URL** with a human-readable message (condition, value, host, timestamp).
    - [ ] If no webhook URL is configured, alerting is **disabled silently** (the dashboard runs exactly as today) — the feature is opt-in.
    - [ ] A delivery failure (non-2xx / timeout) is logged and retried with bounded backoff; it never crashes the dashboard or blocks the poll loop.
...
- **As the operator, I want** one alert per problem, not a flood, **so that** notifications stay meaningful.
  - **Acceptance Criteria:**
    - [ ] An alert fires on the **transition** into the bad state, not on every poll while it persists (edge-triggered).
    - [ ] While a condition stays bad, **re-notification is suppressed** for at least a cooldown window (default 30 minutes) `[NEEDS CLARIFICATION: confirm cooldown default]`.
    - [ ] When a condition returns to normal, a single **recovery** notification is sent and the condition re-arms.
    - [ ] Alert state is per-condition (and per-client for peer-down); one noisy condition doesn't suppress others.
    - [ ] After a dashboard restart, conditions re-arm from current state (no replay of historical alerts); a still-bad condition may re-notify once after restart — acceptable.
...
- **As the operator, I want to** see current alert status in the dashboard, **so that** the push and the pull views agree.
  - **Acceptance Criteria:**
    - [ ] The dashboard surfaces currently-active alerts (e.g. a small banner or an Events/About entry) and whether alerting is configured/enabled.
    - [ ] No new tab is required; reuse the existing Events/Overview surfaces. `[NEEDS CLARIFICATION: confirm placement — proposed: an "active alerts" strip on Overview + entries in Events.]`
...
- **As the operator, I want** alerting to add no inbound exposure, **so that** the security posture is unchanged except for the outbound webhook.
  - **Acceptance Criteria:**
    - [ ] The webhook URL is treated as a **secret** — never rendered into the dashboard UI, never logged in full, never committed to the repo.
    - [ ] Alerting adds only **outbound** HTTPS; no new inbound listener, no in-band auth change (still VPN-gated `http://172.16.15.1:8080`).
    - [ ] Alert messages contain operational data only (condition, host id, metric value) — no secrets, no client private data beyond the client name already shown in the UI.
