...
| `serverinfo` (reuse) | `dashboard/internal/serverinfo/` | Already provides the public IP (IMDSv2); consumed read-only. |
...
- **`503`** — server public key or public endpoint unavailable at request time (do **not** emit a config with a blank/placeholder server field; §2.5 of the functional spec).
...
```
Build(client Client, mode Mode, serverPubKey string, endpoint string) (string, error)
```

- `Client` carries `Name` and `Address` (from `clients.json`). `endpoint` is `"<eip>:51820"`.
- The function emits a fixed template; **only `AllowedIPs` varies by `mode`**:
...
| `[Interface] DNS` | `10.23.0.2` |
| `[Peer] PublicKey` | `serverPubKey` |
| `[Peer] Endpoint` | `endpoint` (`<eip>:51820`) |
| `[Peer] AllowedIPs` (full) | `0.0.0.0/0, ::/0` |
| `[Peer] AllowedIPs` (split) | `172.16.15.0/24, 10.23.0.0/16` |
| `[Peer] PersistentKeepalive` | `25` |

- Constants (`DNS = 10.23.0.2`, ports, CIDRs, keepalive) live as named constants in the package, not scattered string literals — single source of truth, easy to grep when CIDRs change.
- The function is **pure**: no `os`, no `exec`, no network. All host interaction happens in the handler and is passed in as plain strings. This is what makes the builder fully table-testable.
...
- **System Dependencies:**
  ...
  - IMDSv2 public-IP lookup via `internal/serverinfo` — already in use for the About/Overview tabs.
...
- **Unit (the core):** table-driven tests for `wgconfig.Build` covering Full and Split modes — assert the **exact** `.conf` output byte-for-byte, including the placeholder line, `DNS`, both `AllowedIPs` variants, and that no real key ever appears. This is the highest-value test surface and needs no host/network.
- **Handler tests:** using `net/http/httptest`, cover: known client + `mode=full`, `mode=split`, omitted/garbage `mode` (defaults to full), unknown client → `404`, and missing server key / endpoint → `503`. Inject fakes for the `wg` public-key and `serverinfo` endpoint lookups so the handler is tested without `sudo`/IMDSv2.
- **Filename assertion:** verify `Content-Disposition` yields `wg-<name>.conf`.
- **Manual end-to-end (cannot be fully automated in-session):** on a deployed instance, download both modes for a real client, paste the matching private key, and confirm the tunnel connects and routes (full = internet via VPN; split = VPC/peer reachable, local internet untouched, `10.23.0.2` resolves). Per project policy, this manual step must actually be performed before claiming the feature works end-to-end — a passing build does not prove the tunnel comes up.
