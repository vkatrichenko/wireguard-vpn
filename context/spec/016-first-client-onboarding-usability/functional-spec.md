...
- **As an operator, I want `install.sh` to print an example client config, so that I can connect the first client without hand-assembling it or reaching the dashboard first.**
  - **Acceptance Criteria:**
    - [ ] After the "WireGuard server is up" summary, the success output includes a clearly labeled **"Example client config"** block in valid `wg-quick` format (`[Interface]` + `[Peer]`).
    - [ ] The block uses **real server-derived values**: the server's public key (already computed in the script), the listen port, and the **first tunnel IP derived from `WG_SERVER_NET`** (e.g. `172.16.15.2/32` for the default subnet; the correct first host for a custom subnet).
    - [ ] The `Endpoint` is shown as `<server-public-ip>:<port>` (placeholder for the host's public IP) and `PrivateKey` is a clearly-marked placeholder — it is an **example/template**, nothing is auto-generated.
    - [ ] **No client keypair is generated and no private key is ever created or stored on the server.**
    - [ ] The block includes a one-line hint on how to generate the client keypair off-host (`wg genkey`) and how to register the client's public key (via the dashboard, or `WG_PEERS`).
    - [ ] The block is printed for both the dashboard and WG-only installs (it describes the WireGuard server, which always exists).
...
- **As an operator, I want to edit a client in place within its row, so that the fields line up with the client I'm editing instead of a cramped side panel.**
  - **Acceptance Criteria:**
    - [ ] Clicking **Edit** reveals an editable form **within that client's expanded row** (same column as the listing); the separate right-side drawer is no longer used for editing.
    - [ ] The form exposes the same editable fields as today (name, public key, tunnel IP, note, enable/disable) with the same validation.
    - [ ] **Save** applies the change live (same no-downtime apply as today) and collapses back to the row's read view; **Cancel** discards changes with no modification.
    - [ ] Validation errors render **inline within the row form**, and no partial change is applied on rejection.
    - [ ] Editing one client does not disrupt other rows, the client list, or the geo map.
...
- **As an operator, I want the handshakes panel to show client names, one row per peer, so that I can see at a glance who was last connected.**
  - **Acceptance Criteria:**
    - [ ] Each row shows the **client name** (resolved from the public key via the client list) instead of the raw public key.
    - [ ] Each peer appears **at most once**, showing its **most-recent** handshake time; the repeated rows for the same peer are gone.
    - [ ] Rows are ordered **most-recent first**.
    - [ ] A handshake from a public key **not** in the client list still appears, with a clear fallback label (e.g. a shortened key marked "unknown") rather than being hidden.
...
- **As an operator, I want to install, update, and cleanly remove WireGuard/dashboard with the same script, so that I can manage the box's lifecycle without manual teardown or data loss.**
  - **Acceptance Criteria:**
    - [ ] **Install (fresh):** running the script with no action flag on a clean host installs WireGuard (+ dashboard when a release is pinned), exactly as today.
    - [ ] **Update (rerun):** running the script with no action flag on an **already-installed** host updates everything in place — re-downloads/installs the dashboard binary and rewrites the units/helper/script — and:
      - [ ] reuses the existing `/etc/wireguard/server.key` (no need to pass `WG_SERVER_PRIVATE_KEY`); the server's public key/identity is unchanged.
      - [ ] preserves the dashboard client DB (runtime-added clients survive).
      - [ ] does **not** overwrite a dashboard-managed `wg0.conf` with the (possibly empty) `WG_PEERS` env — the live peer set is not clobbered. _(This fixes the observed "wg peers gone after rerun" defect.)_
      - [ ] does not drop active tunnels for the update.
      - [ ] the running dashboard process is actually replaced by the new binary (the `enable --now` → `restart` fix).
    - [ ] **Remove (`--uninstall`):** stops and disables the `wireguard-dashboard` and `wg-quick@wg0` services and removes the installed artifacts (dashboard binary, systemd units, `wg-sync` helper, sudoers entry, dashboard system user). By default it **keeps data** — the server key, `wg0.conf`, and the dashboard client DB remain, so a later reinstall keeps the same server identity.
    - [ ] **Dashboard-only remove:** a narrower remove mode tears down only the dashboard (service, unit, binary, helper, sudoers, user) and leaves the WireGuard tunnel running.
    - [ ] **Purge (`--purge`):** in addition to remove, deletes the server key, `wg0.conf`, and the dashboard client DB — a full wipe (a reinstall then gets a new server key; existing client configs break).
    - [ ] Remove/purge are **idempotent** and safe to run when components are already absent (no hard failure on missing units/files).
    - [ ] The script remains `set -euo pipefail` and shellcheck-clean across all actions.
