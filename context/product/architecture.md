...
- **Dashboard is always installed (spec 019):** `install.sh` **always** provisions the dashboard alongside WireGuard — the WG-only install path was removed (other tooling covers a bare WG server), so `dashboard_release_tag` (EC2) / `DASHBOARD_RELEASE_TAG` (VPS) is now **required** and fails hard if empty.
- **Client Management (spec 019 — the UI is the sole peer authority):** the dashboard's on-box SQLite `clients` table is the single source of truth for peers, mutated **only** through the dashboard UI (or the equivalent on-box `wg-peer` CLI, below). Operators add / remove / edit clients live from the Clients tab (paste-public-key; auto `/32` allocation), applied to the running interface with **no instance replacement and no tunnel drop** via a dashboard-staged peers-only file + a single root `wg-sync` helper that runs `wg syncconf`. **Terraform no longer declares the peer set** — it seeds exactly one **`admin_peer`** (name + **public** key; nullable) for anti-lockout, and **only when the store is empty** (afterward it is an ordinary UI-editable peer; it never resurrects a removed admin). There is **no `clients_config` list, no drift `check`, no HCL/tfvars export, and no drift badge** — all removed with spec 019, superseding the spec-017 `restapi` bulk-`PUT` and spec-018 S3-bridge iterations. Two storage modes: **`local`** = on-box SQLite only, no external store (the standalone-VPS default); **`cloud`** = SQLite **plus a versioned S3 object as a pure durable backup** — the dashboard write-throughs every mutation to `s3://…/clients.json` and restores from it on boot, surviving instance rebuild, while **Terraform never reads or reconciles it** (no drift detection; it only provisions the bucket + IAM — see §4). A missing/empty S3 object is **never** treated as authoritative (never a wipe); a hard S3 error latches write-through off (`storeReady=false`) and self-heals on the next successful reconcile. Behaves identically on EC2 and standalone VPS.
- **On-box `wg-peer` CLI (spec 019):** an optional `/usr/local/bin/wg-peer` (`add` / `remove` / `update`) is a **thin client over the local dashboard HTTP API** (`POST` / `PATCH` / `DELETE /api/clients`), so it is the same mutation path as the UI by construction (IP allocation, `wg syncconf`, S3 write-through, config generation all run in the dashboard). Its purpose is first-peer onboarding on a manual VPS without SSH-forwarding the VPN-only dashboard. `add` **server-generates an ephemeral keypair** by default (kept in a shell variable only); `--show-config` prints the full client config — private key included — then discards it (never written to disk/SQLite/S3), while `--pubkey` keeps the strict bring-your-own-key path. Installed by fetching `scripts/wg-peer` from raw GitHub at the same pinned ref as `install.sh`.
...
- **Client Keys:** Generated off-host (`wg genkey | tee privatekey | wg pubkey > publickey`); only the public key is registered (pasted into the dashboard UI, seeded as the `admin_peer` public key, or added to `WG_PEERS`). **The server never holds a client private key** — with one scoped, documented exception: `wg-peer add` (no `--pubkey`) generates an **ephemeral** keypair, prints it once via `--show-config`, and immediately discards the private key without persisting it (§3).
- **IAM Access:** Instance role with scoped SSM `GetParameter` permission for the private key parameter.
- **Client-list backup (cloud mode, spec 019):** a dedicated **versioned** S3 bucket (`<project>-<env>-wireguard-client-list`, SSE-S3, public-access-blocked, `force_destroy`) holds `clients.json` as a **non-authoritative backup** — Terraform seeds no object (the dashboard creates it on first boot). The instance role grants least-privilege **`s3:GetObject` + `s3:PutObject` on the single `clients.json` object, plus `s3:ListBucket` on the bucket**. `ListBucket` is **required, not cosmetic**: because the dashboard (not Terraform) creates the object, the first-boot `get-object` on the not-yet-existing key returns `404 NoSuchKey` **only** when the caller holds `ListBucket`; without it S3 returns `403`, which the dashboard classifies as a hard error and stops writing — silently disabling the backup. `local` mode provisions none of this (all resources are cloud-gated).
...
- **Purpose:** VPN-only ops dashboard — peer status, throughput, connection history, peer geo map, and proactive alerting (specs 002 / 003 / 006 / 007). **Read-only except** the runtime webhook-management write endpoints (008, §8) and the runtime **client-management** write endpoints (`POST/PATCH/DELETE /api/clients`, specs 015/019; see §3) — the dashboard is now the **sole** authority for the peer set, and `wg-peer` drives these same routes. **No authentication** — access is gated solely by the WireGuard tunnel.
...
- **Views:** Overview / Clients / System / Network / Events / About tabs. Overview carries an **active-alerts strip** (007); per-client expand panel = throughput chart + connection timeline + history summary; offline world-map card (embedded SVG + equirectangular markers, bounded zoom/pan) on the Clients tab (006/010); **webhook-management card on About** (008). The **Clients tab** carries runtime **add / inline-edit / enable-disable / remove** controls (full-width in-row edit, spec 016) — spec 019 **removed the git-reconciliation UI** (the HCL/tfvars export link and the drift badge) since there is no longer a declarative peer set to reconcile against; the **Events tab** "Recent handshakes" resolves public keys to **client names, one row per peer** (spec 016).
- **Host data sources:** the **client list comes from the SQLite `clients` table** (`clients.json` on disk is only the first-boot seed — the single `admin_peer` or empty — and, in cloud mode, mirrors the S3 backup object; there is no drift baseline). Live apply goes through a single root **`wg-sync` helper** (one exact-match NOPASSWD sudoers entry) that merges a dashboard-staged **peers-only** file into `wg0.conf` and runs `wg syncconf` — the server key never leaves root. Read state via scoped NOPASSWD `wg show wg0 dump`/`public-key` + `systemctl is-active/show wg-quick@wg0`. Public-IP / server-endpoint info comes from **IMDSv2 on EC2**, falling back off-AWS to an operator-set `WG_PUBLIC_ENDPOINT` or an external echo (`WG_PUBLIC_IP_ECHO_URL`, default `api.ipify.org`); EC2-only About cards degrade to "Not running on EC2" rather than erroring (spec 016). Generated client configs use a VPC-derived DNS on EC2 and `WG_CLIENT_DNS` (default `1.1.1.1`) off-AWS. **Never holds client private keys.**

---

## 7. Dashboard Build & Deployment

- **Distribution (spec 005 / 013):** public GitHub Release artifact from `vkatrichenko/wireguard-vpn`, pinned via `dashboard_release_tag` in `terraform/dev/main.tf` (currently **`v0.0.16`**) — single reviewable source of truth, same explicit-pin philosophy as the AMI. Each release publishes **both** `wireguard-dashboard-amd64` and `wireguard-dashboard-arm64` (spec 013). Replaced the earlier private S3-artifact + ECR path.
- **Install (via `scripts/install.sh`):** the installer downloads `wireguard-dashboard-$GOARCH` from the pinned release and installs it (no checksum step — see §3). Since spec 019 the **dashboard is always installed** with WireGuard (no WG-only path), so `dashboard_release_tag` (EC2) / `DASHBOARD_RELEASE_TAG` (VPS) is **required** and fails hard if empty. On EC2 this runs inside the fetched shared script (spec 014); on a VPS it is the same code path. The installer also fetches `scripts/wg-peer` from raw GitHub at the same pinned ref (§3) and installs `jq`. A **rerun `restart`s** the unit so the new binary takes effect (spec 016).
...
- **Binding & access:** `LISTEN_ADDR=172.16.15.1:8080` — bound to the WireGuard tunnel IP, so reachable only over the VPN (no public listener; this is why no in-band auth is needed, including for the 008 + 015/019 write endpoints — the on-box `wg-peer` CLI reaches the same local API).
...
## 9. Implementation Status Snapshot (2026-07-03)

- Specs 002–016 deployed and operator-verified. **Spec 017** (Terraform-managed peers via `restapi` bulk-PUT) and **spec 018** (`client_management_mode` local|cloud, S3-bridge) shipped and were then **superseded by spec 019** — their machinery (the `restapi` provider + `manage_peers_via_api`, the warn-only S3 drift `check`, the `managed_baseline` table + `PUT /api/clients` bulk endpoint, the HCL/tfvars export + drift badge) is **removed**. **Spec 019 (UI-first client management)** is the current model: the dashboard UI (+ `wg-peer` CLI) is the sole peer authority, Terraform seeds one `admin_peer`, and `cloud` mode keeps S3 as a pure backup (requiring `s3:ListBucket`, §4). Implemented & operator-verified live 2026-07-03; current dashboard release **`v0.0.16`** (PR #55).
