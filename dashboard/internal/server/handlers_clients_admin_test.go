...
// server.New(..., metricsDB, ...) are passed the exact same *db.DB, so a test
// harness that (mistakenly) wired two separate DBs would see writes through
// clientsSvc "disappear" from the server's view. All other deps are the
// package's existing fakes — these tests only drive the client-management
// surface. The wg fake returns no peers, so any added client renders
// "pending".
