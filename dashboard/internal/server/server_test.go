...
func TestHandleGetService_IncludesEvents(t *testing.T) {
	...
	// nil geo resolver is allowed — geo is advisory.
	handler, err := server.New(dashboard.WebFS(), infoSvc, &systemdSvc, fakeClientsfileSvc(), fakeWgSvc(), fakeProcSvc(), testDB, nil, fakeDiskSvc(), fakeProcessesSvc(), fakeNetdevSvc(), nil, nil, nil, emptyClientsSvc(t), "local", nil)
	if err != nil {
		t.Fatalf("server.New: %v", err)
	}
...
func TestHandleGetPartialSystem_RendersDiskCard(t *testing.T) {
	...
	handler, err := server.New(dashboard.WebFS(), infoSvc, &systemdSvc, fakeClientsfileSvc(), fakeWgSvc(), fakeProcSvc(), newTestDB(t), nil, amberDiskSvc, fakeProcessesSvc(), fakeNetdevSvc(), nil, nil, nil, emptyClientsSvc(t), "local", nil)
	if err != nil {
		t.Fatalf("server.New: %v", err)
	}
...
func TestHandleGetPartialClients_SeededRow(t *testing.T) {
	...
	handler, err := server.New(dashboard.WebFS(), infoSvc, &systemdSvc, clientsSvc, wgSvc, fakeProcSvc(), newTestDB(t), geo, fakeDiskSvc(), fakeProcessesSvc(), fakeNetdevSvc(), nil, nil, nil, seededClientsSvc(t, db.Client{Name: peerName, Address: peerAddress, PublicKey: peerPublicKey}), "local", nil)
	if err != nil {
		t.Fatalf("server.New: %v", err)
	}
...
func TestHandleGetPartialClients_SeededRow_PubkeyWithSpecialChars(t *testing.T) {
	...
	handler, err := server.New(dashboard.WebFS(), infoSvc, &systemdSvc, clientsSvc, wgSvc, fakeProcSvc(), newTestDB(t), nil, fakeDiskSvc(), fakeProcessesSvc(), fakeNetdevSvc(), nil, nil, nil, seededClientsSvc(t, db.Client{Name: peerName, Address: peerAddress, PublicKey: peerPublicKey}), "local", nil)
	if err != nil {
		t.Fatalf("server.New: %v", err)
	}
...
func TestHandleGetPartialClientDetail_404OnUnknown(t *testing.T) {
	...
	handler, err := server.New(dashboard.WebFS(), infoSvc, &systemdSvc, clientsSvc, fakeWgSvc(), fakeProcSvc(), newTestDB(t), nil, fakeDiskSvc(), fakeProcessesSvc(), fakeNetdevSvc(), nil, nil, nil, emptyClientsSvc(t), "local", nil)
	if err != nil {
		t.Fatalf("server.New: %v", err)
	}
...
func TestHandleGetMetricsProm(t *testing.T) {
	...
	handler, err := server.New(dashboard.WebFS(), infoSvc, &systemdSvc, fakeClientsfileSvc(), fakeWgSvc(), fakeProcSvc(), newTestDB(t), nil, fakeDiskSvc(), fakeProcessesSvc(), fakeNetdevSvc(), nil, nil, provider, emptyClientsSvc(t), "local", nil)
	if err != nil {
		t.Fatalf("server.New: %v", err)
	}
