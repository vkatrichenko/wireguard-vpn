...
| <a name="input_install_script_ref"></a> [install\_script\_ref](#input\_install\_script\_ref) | A commit SHA or tag pinning the exact scripts/install.sh version fetched at boot from install\_script\_repo. Must point at a ref on the PUBLIC default branch where the install.sh matching install\_script\_sha256 exists, otherwise the raw fetch 404s and boot aborts. Bumping this re-renders user-data, rolls a new launch-template version, and replaces the instance. | `string` | `"main"` | no |
