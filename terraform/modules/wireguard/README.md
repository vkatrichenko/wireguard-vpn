...
| <a name="input_dashboard_alerts"></a> [dashboard\_alerts](#input\_dashboard\_alerts) | Spec-007 alert thresholds seeded into /etc/wireguard-dashboard/alerts.env (mapped to DASHBOARD\_HOST\_LABEL / DASHBOARD\_ALERT\_DISK\_PCT / \_CPU\_PCT / \_CPU\_SUSTAIN / \_PEER\_STALE / \_TRANSFER\_BYTES). host\_label empty (default) omits DASHBOARD\_HOST\_LABEL so the Go side falls back to os.Hostname(). cpu\_sustain/peer\_stale are Go durations (e.g. "5m"); transfer\_bytes is a humanized size (e.g. "50GiB"). | <pre>object({<br/>    host_label     = optional(string, "")<br/>    disk_pct       = optional(number, 90)<br/>    cpu_pct        = optional(number, 90)<br/>    cpu_sustain    = optional(string, "5m")<br/>    peer_stale     = optional(string, "10m")<br/>    transfer_bytes = optional(string, "50GiB")<br/>  })</pre> | `{}` | no |
